...
class vLLMRollout(BaseRollout):
    ...
    @torch.no_grad()
    def generate_sequences_collector(self, prompts: DataProto, **kwargs) -> DataProto:
            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    ...
                    if self.config.calculate_log_probs:
                        curr_log_prob = []
                        curr_candidate_log_prob = []
                        curr_candidate_token_id = []
                        for i, logprob in enumerate(output.outputs[sample_id].logprobs):
                            curr_candidate_token_id.append(list(logprob.keys())[1:])
                            curr_candidate_log_prob.append([logprob[k].logprob for k in list(logprob.keys())[1:]])
                            curr_log_prob.append(logprob[response_ids[i]].logprob)
                        rollout_log_probs.append(curr_log_prob)
                        candidate_log_probs.append(curr_candidate_log_prob)
                        candidate_token_ids.append(curr_candidate_token_id)
            
            non_tensor_batch["candidate_token_ids"] = np.array(candidate_token_ids, dtype=object)
            non_tensor_batch["candidate_log_probs"] = np.array(candidate_log_probs, dtype=object)
    ...
    @GPUMemoryLogger(role="vllm rollout spmd", logger=logger)
    @torch.no_grad()
    def generate_sequences_tspo(self, prompts: DataProto, **kwargs) -> DataProto:
        """Generate sequences for a batch of prompts.
        ...
        with self.update_sampling_params(**kwargs):
            ...
            for i in range(len(response)):
                ...
                if self.config.calculate_log_probs:
                    rollout_log_probs[i] = [1.0] * len(partial_solution[i]) + rollout_log_probs[i]
                    if len(rollout_log_probs[i]) > self.config.response_length:
                        rollout_log_probs[i] = rollout_log_probs[i][: self.config.response_length]
